...
const file = `${path.sep}test.json`;


describe('Note Service with proper file', () => {
    beforeEach(function(done) {
        fs.unlink(file, () => {
            this.noteService = new NoteService('test.json');
            done();
        });
    });
    ...
    it('should be able to add a note', function(done) {
        this.noteService.addNote('one note', 'sam')
    ...
    it('should be able to update a note', function(done) {
        this.noteService.addNote('one note', 'sam')
            .then(() => this.noteService.updateNote(0, 'good note', 'sam'))
            .then(() => this.noteService.read())
            .then((notes) => {
                expect(notes).toEqual({
                    sam: ['good note']
    ...
    it('should be able to remove a note', function(done) {
        this.noteService.addNote('one note', 'sam')
            .then(() => this.noteService.deleteNote(0, 'sam'))
            .then(() => this.noteService.read())
            .then((notes) => {
                expect(notes).toEqual({
                    sam: []
    ...
    it('should throw an error for updating a non-existent user', function(done) {
        this.noteService.addNote('one note', 'sam')
            .then(() => this.noteService.updateNote(0, 'good note', 'altaf'))
    ...
    it('should throw an error for removing a non-existent user', function(done) {
        this.noteService.addNote('one note', 'sam')
            .then(() =>this.noteService.deleteNote(0, 'altaf'))
    ...
    it('should throw an error for updating a non-existent note', function(done) {
        this.noteService.addNote('one note', 'sam')
            .then(() => this.noteService.updateNote(1, 'good note', 'sam'))
            .then(() => done.fail('Expect updating a non-existent to be failed'))
            .catch((err) => {
    ...
    it('should throw an error for removing a non-existent note', function(done) {
        this.noteService.addNote('one note', 'sam')
            .then(() => this.noteService.deleteNote(1, 'sam'))
            .then(() => done.fail('Expect removing a non-existent to be failed'))
            .catch((err) => {
    ...
    describe('with many notes added', () => {
        beforeEach(function(done) {
            this.noteService.addNote('one note', 'sam')
                .then(() => this.noteService.addNote('two note', 'sam'))
                .then(() => this.noteService.addNote('another note', 'altaf'))
                .then(() => this.noteService.addNote('separated', 'altaf'))
                .then(done)
                .catch(done.fail);
        });
        ...
        it('should be able to remove a particular note', function(done) {
            this.noteService.deleteNote(0, 'sam')
                .then(() => this.noteService.deleteNote(1, 'altaf'))
                .then(() => this.noteService.read())
                .then((notes) => {
                    expect(notes).toEqual({
                        sam: ['two note'],
        ...
        it('should be able to update a particular note', function(done) {
            this.noteService.updateNote(0, 'good note', 'sam')
                .then(() => this.noteService.updateNote(1, 'great note', 'altaf'))
                .then(() => this.noteService.read())
                .then((notes) => {
                    expect(notes).toEqual({
